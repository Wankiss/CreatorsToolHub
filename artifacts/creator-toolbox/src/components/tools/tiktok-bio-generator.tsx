    ...
    <div className="space-y-6">
      <Card className="p-6 space-y-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="font-semibold text-lg">TikTok Bio Generator</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="text-sm font-medium mb-1 block">Your Name</label>
            <Input placeholder="e.g. Alex, FitWithSarah..." value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">Your Niche *</label>
            <Input placeholder="e.g. fitness, cooking, travel, finance..." value={niche} onChange={e => setNiche(e.target.value)} onKeyDown={e => e.key === "Enter" && handleGenerate()} />
          </div>
        </div>
          ...
          <div className="space-y-3">
            {outputs.map((bio, i) => (
              <div key={i} className="group p-4 rounded-lg border bg-muted/30 hover:bg-muted/60 transition-colors">
                ...
                <p className="text-xs text-muted-foreground mt-1">{bio.length} chars</p>
              </div>
            ))}
          </div>
        </Card>

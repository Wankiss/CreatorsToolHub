  ...
  const handleExecute = (e: React.FormEvent) => {
    ...
    executeMutation.mutate({ 
      ...
      onSuccess: (res) => {
        if (res.success && res.outputs.length > 0) {
          ...
          toast({
            ...
            description: "No outputs were generated. Please try a different prompt.",
            variant: "destructive"
          });
        }
      },
      onError: () => {
        toast({
          title: "Error",
          description: "Something went wrong communicating with the AI. Try again later.",
          variant: "destructive"
        });
      }
    });
      ...
      <div className="bg-muted/30 border-b border-border py-4">
        <div className="container mx-auto px-4 max-w-7xl">
          <nav className="flex items-center text-sm font-medium text-muted-foreground space-x-2">
            <Link href="/" className="hover:text-primary transition-colors flex items-center"><Home className="w-4 h-4 mr-1"/> Home</Link>
            <ChevronRight className="w-4 h-4" />
            <Link href={`/category/${tool.categorySlug}`} className="hover:text-primary transition-colors flex items-center"><LayoutGrid className="w-4 h-4 mr-1"/> {tool.categoryName}</Link>
            <ChevronRight className="w-4 h-4" />
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="flex-1 space-y-12">
            ...
            <section>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h1 className="text-3xl md:text-4xl font-display font-bold tracking-tight text-foreground">{tool.name}</h1>
                  <div className="flex items-center gap-3 mt-1.5 text-sm text-muted-foreground">
                    <span className="flex items-center"><Activity className="w-4 h-4 mr-1 text-primary"/> {tool.usageCount.toLocaleString()} uses</span>
                    <span>•</span>
                    <span className="text-primary font-medium">Free Tool</span>
                  </div>
                </div>
              </div>
              <p className="text-lg text-muted-foreground mb-8 text-balance">{tool.description}</p>
                ...
                <div className="bg-card rounded-[1.35rem] p-6 sm:p-8">
                  <form onSubmit={handleExecute} className="space-y-6">
                    <div className="space-y-3">
                      ...
                      <Textarea 
                        placeholder="E.g., I'm making a video about 10 passive income ideas for beginners in 2024..." 
                        className="min-h-[160px] text-base resize-y p-4 bg-muted/50 focus-visible:ring-primary/30 border-muted-foreground/20 rounded-xl"
                        value={input}
            ...
            {outputs.length > 0 && (
              <section id="results-area" className="scroll-mt-24 animate-in slide-in-from-bottom-8 duration-700">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="bg-primary/10 text-primary text-xs font-bold px-3 py-1 rounded-full">{outputs.length} options</span>
                </div>
            ...
            <div className="adsense-placeholder w-full h-[280px] my-12" />
            ...
            <div className="prose prose-gray dark:prose-invert max-w-none prose-headings:font-display prose-h2:text-2xl prose-h3:text-xl">
              {tool.howToGuide && (
                <section className="mb-12">
                  <h2 className="flex items-center gap-2 border-b pb-4"><HelpCircle className="text-primary" /> How to use the {tool.name}</h2>
              ...
              {tool.seoContent && (
                <section className="mb-12">
                  <h2 className="flex items-center gap-2 border-b pb-4"><FileText className="text-primary" /> About this Tool</h2>
              ...
              {tool.faqContent && (
                <section className="mb-12">
                  <h2 className="flex items-center gap-2 border-b pb-4"><HelpCircle className="text-primary" /> Frequently Asked Questions</h2>
          ...
          <aside className="w-full lg:w-[336px] flex-shrink-0 space-y-8">
            <div className="sticky top-24 space-y-8">
              ...
              {/* Top Sidebar AdSense Placeholder */}
                    ...
                    {tool.relatedTools.map(t => (
                      <Link key={t.id} href={`/tools/${t.slug}`}>
                        <div className="flex items-start gap-3 p-3 rounded-xl hover:bg-background border border-transparent hover:border-border hover:shadow-sm transition-all cursor-pointer group">
                          ...
                          <div>
                            <h4 className="font-bold text-sm text-foreground group-hover:text-primary transition-colors">{t.name}</h4>
              ...
              {/* Bottom Sidebar AdSense Placeholder */}

...
function buildHowToSchema(
  ...
  // Only generate for how-to and step-by-step posts
  if (!slug.includes("how-to") && !slug.includes("-steps") && !slug.includes("-checklist-")) return null;
  ...
  // Extract each H2/H3 and the first <p> immediately following it
  const stepMatches = [...content.matchAll(/<h[23][^>]*>(.*?)<\/h[23]>\s*(?:<p[^>]*>(.*?)<\/p>)?/gis)];
  if (stepMatches.length < 2) return null;

  const steps = stepMatches
    .filter(m => {
      const name = stripTags(m[1] ?? "");
      return name && !/^(FAQ|Frequently Asked|Conclusion|Final Thoughts|Summary|Wrapping Up|Key Takeaways)/i.test(name);
    })
    .slice(0, 10)
    .map((m, i) => ({
      "@type":    "HowToStep",
      "position": i + 1,
      "name":     stripTags(m[1] ?? ""),
      "text":     m[2] ? stripTags(m[2]).slice(0, 300) : stripTags(m[1] ?? ""),
    }));
...
export async function resolvePageMeta(rawPathname: string): Promise<PageMeta | null> {
  ...
  if (blogMatch) {
    ...
    try {
      ...
      // HowTo schema — injected for posts whose slug signals step-by-step content.
      // Steps are extracted from H2/H3 headings so AI systems see structured steps.
      const howToSchema = buildHowToSchema(slug, post.title, description, post.content ?? "", imageUrl, canonical);
